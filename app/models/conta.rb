class Conta < ActiveRecord::Base
  ...
  enum situacao: [:cancelado, :ativo, :bloqueado]

  validates :nome, presence: true,
                   uniqueness: true,
                   format: { with: /\A[^0-9ç!@\$%\^&*+_=]+\z/ },
                   length: { in: 2..70 }
  ...
  def conta_valida?
    if self.situacao == 'cancelado' || self.situacao == 'bloqueado'
      errors.add(:conta, message: 'não podem ser efetuadas transações enquanto a conta estiver cancelada/bloqueada')
      false
    else
      true
    end
  ...
  private
    ...
    def conta_cancelada_nao_recebe_atividade
      ...
      if conta_antes_atualizacao.status == 'cancelado'
        errors.add(:conta, message: 'conta cancelada não sofre movimentações')
        return false
      end
